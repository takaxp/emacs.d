...
4.  [カーソル移動](#orgca7e422)
    ...
    12. [[smart-mark] C-g後に元の場所へカーソルを戻す](#org9163094)
...
6.  [表示サポート](#orgcb45379)
    ...
    10. [[yascroll] フリンジにスクロールバーを出す](#org6d27d69)
...
9.  [開発サポート](#orgbe5dd23)
    ...
    7.  [[auto-complete-clang.el] オムニ補完](#org128ec20)
...
-   [casouri/isolate: Surrounding magics, extensible](https://github.com/casouri/isolate)
...
## TODO [bratex] LaTeX 数式のブラケット入力をサポート

-   [sbrisard/bratex: Emacs package for manipulation of brackets in LaTeX mode](https://github.com/sbrisard/bratex)
...
## TODO [dired-recent] 訪問したディレクトリの履歴を取る
...
-   後発として [bicycle.el](https://github.com/tarsius/bicycle) があります．
...
-   [format-all - MELPA](https://melpa.org/#/format-all)

様々なプログラミング言語に対して，共通のコード整形のトリガー（ `M-x format-all-buffer` ）を提供します．整形するために使うフォーマッタは，原則的に言語ごとに異なる外部ツールを呼び出すので，必要に応じてそれらのツールのインストールが別途必要です．  
...
それ以外にも [ox-s5](https://github.com/cybercode/org-slides)，[org-ioslide](https://github.com/coldnew/org-ioslide) があります．  
...
<del>（注）Growlnotify の代わりに `terminal-notifier` を使うこともできます．</del>  
...
`moom-autuloads` に必要なコマンドを <del>羅列していますが，</del> `melpa` でインストールする場合は関係ありません．パッケージ内で autoload 指定してあります． `with-eval-after-load` の中だけを設定すれば十分です．  
...
私は使っていませんが，簡単にテーマを切り替えるためのパッケージ([heaven-and-hell](https://github.com/valignatev/heaven-and-hell))も存在します．  
...
-   <del>色々と Linux用に設計・前提とされていて，今のところ Macで使うのはちょっと厳しい．</del>  
